...
(function(dot){
	var pageMore = function(){
		...
		updateKetdata = function(){
			var jumlah = $('.tampil-kategori tbody tr').length;
			$('#ket-index-data')
		...
		return {
			result:function(){
				...
				var url = Datadefine['url']+"index.php/c_function/panggil_data_kategori";
				ajaxRequest({
					type: "get",
	          		url: url,
	          		...
	          		success:function( result ){
	          			...
	          			$('.tampil-kategori tbody').append( data.data );
	          			btn.disabled = false;
	...
	var editReck = function(){
		return {
			result:function(){
				...
				$('.modal-warning #form-input-kategori .id').val(id);
				$('.modal-warning #form-input-kategori .nama').val(nama);
				$('.modal-warning #form-input-kategori .keterangan').val(keterangan);
...
$('#kirim').click(function(e){
	e.preventDefault();
	var form_data = $('.modal-primary #form-input-kategori').serialize();
	var URL = Datadefine['url']+'index.php/c_function/input_data_kategori';
	ajaxRequest({
		type:'post',
		url:URL,
		data:form_data,
		...
		success:function(result){
			...
			if(data.result){
				...
				$('.modal-primary #form-input-kategori')[0].reset();
			}else{
...
$('#update').click(function(e){
	ajaxRequest({
		type:'get',
		url:Datadefine['url']+"index.php/c_function/update_kategori",
		data:$('.modal-warning #form-input-kategori').serialize(),
		beforeSend:function(){
...
$('#delete').click(function(e){
	ajaxRequest({
		type:'get',
		url:Datadefine['url']+"index.php/c_function/hapus_kategori",
		data:$('.modal-danger #form-hapus-kategori').serialize(),
		beforeSend:function(){
...
$('#refresh-barang').click(function(e){
	...
	ajaxRequest({
		type:'get',
		url:Datadefine['url']+"index.php/c_function/refresh_kategori",
		data:'',
		beforeSend:function(){
			$('#refresh-box').show();
		},success:function(result){
			...
			$('#page-more').html('');
			if(r.countData > 0){
				$('table.tampil-kategori tbody').html(r.data);
				$('#ket-pencarian').html('');
				updateIndexData();
				if(r.next == "ada"){
					updatePageMoreBtn();

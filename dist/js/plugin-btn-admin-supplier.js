	...
	var pageMore = function(){
		...
		return {
			result:function(){
				...
				var url = Datadefine['url']+"index.php/c_supplier/panggil_data_supplier";
				ajaxRequest({
					type: "get",
	          		url: url,
	          		...
	          		success:function( result ){
	          			...
	          			$('.tampil-supplier tbody').append( data.data );
	          			btn.disabled = false;
	...
	var editReck = function(){
		return {
			result:function(){
				let id = arguments[0].target.parentNode.parentNode.children[0].innerHTML;
				let nama = arguments[0].target.parentNode.parentNode.children[1].innerHTML;
				let telp = arguments[0].target.parentNode.parentNode.children[2].innerHTML;
				let alamat = arguments[0].target.parentNode.parentNode.children[3].innerHTML;
				$('.modal-warning #form-input-supplier .id').val(id);
				$('.modal-warning #form-input-supplier .nama').val(nama);
				$('.modal-warning #form-input-supplier .telp').val(telp);
				$('.modal-warning #form-input-supplier .alamat').val(alamat);
			}
		}
	}
...
$('#kirim').click(function(e){
	e.preventDefault();
	var form_data = $('.modal-primary #form-input-supplier').serialize();
	var URL = Datadefine['url']+'index.php/c_supplier/input_data_supplier';
	if($('.modal-primary #form-input-supplier')[0].checkValidity()){
		ajaxRequest({
			type:'post',
			url:URL,
			data:form_data,
			...
			success:function(result){
				...
				if(data.result){
					...
					$('.modal-primary #form-input-supplier')[0].reset();
				}else{
...
$('#update').click(function(e){
	ajaxRequest({
		type:'get',
		url:Datadefine['url']+"index.php/c_supplier/update_supplier",
		data:$('.modal-warning #form-input-supplier').serialize(),
		beforeSend:function(){
...
$('#delete').click(function(e){
	ajaxRequest({
		type:'get',
		url:Datadefine['url']+"index.php/c_supplier/hapus_supplier",
		data:$('.modal-danger #form-hapus-supplier').serialize(),
		beforeSend:function(){
...
$('#refresh-barang').click(function(e){
	...
	ajaxRequest({
		type:'get',
		url:Datadefine['url']+"index.php/c_function/refresh_supplier",
		data:'',
		beforeSend:function(){
			$('#refresh-box').show();
		},success:function(result){
			...
			$('#page-more').html('');
			if(r.countData > 0){
				$('table.tampil-supplier tbody').html(r.data);
				$('#ket-pencarian').html('');
				updateIndexData();
				if(r.next == "ada"){
					updatePageMoreBtn();
